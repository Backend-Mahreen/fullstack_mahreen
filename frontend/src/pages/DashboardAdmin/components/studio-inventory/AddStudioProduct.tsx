...
import { fetchStudioCategories, type StudioCategory } from "../../../../utils/studioCategory";

type AddStudioProductProps = Readonly<{
  onCancel: () => void;
  onSaved: (product: StudioProductRecord) => void;
  editingProduct?: StudioProductRecord;
}>;

const AddStudioProduct = ({ onCancel, onSaved, editingProduct }: AddStudioProductProps) => {
  ...
  const [collection, setCollection] = useState<NewStudioProduct["collection"]>(editingProduct?.collection ?? "Signature");
  ...
  const [discountPrice, setDiscountPrice] = useState(editingProduct?.discountPrice ?? 0);
  const [stock, setStock] = useState(editingProduct?.stock ?? 0);
  const [lowStockThreshold, setLowStockThreshold] = useState(editingProduct?.lowStockThreshold ?? 5);
  const [visibility, setVisibility] = useState<NewStudioProduct["visibility"]>(editingProduct?.visibility ?? "Public");
  const [currentStatus, setCurrentStatus] = useState("Published");
  const [weight, setWeight] = useState(editingProduct?.weight ?? "500g");
  const [dimensions, setDimensions] = useState(editingProduct?.dimensions ?? "L × W × H");
  const [shippingClass, setShippingClass] = useState(editingProduct?.shippingClass ?? "Standard Luxury");
  const [image, setImage] = useState(editingProduct?.image ?? "");
  const [imageUrlMode, setImageUrlMode] = useState(false);
  const [error, setError] = useState("");

  useEffect(() => {
    fetchStudioCategories().then(setCategories);
  }, []);
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    try {
      const product = isEditing
        ? adminEcosystemRepository.updateStudioProduct(editingProduct!.id, {
            name: name.trim(),
        ...
        : adminEcosystemRepository.saveStudioProduct({
            name: name.trim(),
            subtitle: collection === "Limited Edition" ? "Limited Collection" : `${collection} Collection`,
            category,
            sku: sku.trim(),
  ...
  return (
    <form className="admin-product-form admin-feature-enter" onSubmit={handleSubmit}>
      <header className="admin-product-form__heading">
        <div><span className="admin-feature-eyebrow">MAHREEN STUDIO · PRODUCT WORKSPACE</span><h1>{isEditing ? "Edit Produk" : "Tambah Produk Baru"}</h1><p>{isEditing ? "Perbarui informasi produk dalam katalog Mahreen Studio." : "Daftarkan koleksi eksklusif baru ke dalam katalog Mahreen Studio."}</p></div>
        <div><button className="admin-feature-outline-button" type="button" onClick={onCancel}>Batalkan</button><button className="admin-feature-gold-button" type="submit"><Save size={16} /> {isEditing ? "Simpan Perubahan" : "Simpan Produk"}</button></div>
      </header>
      ...
      <div className="admin-product-form__layout">
        <div className="admin-product-form__main">
          <section className="admin-form-card">
            ...
            <div className="admin-form-grid"><label><span>SKU</span><input value={sku} onChange={(event) => setSku(event.target.value)} /></label><label><span>Category</span><select value={category} onChange={(event) => setCategory(event.target.value)}>{categories.length > 0 ? categories.map((cat) => <option key={cat.id} value={cat.name}>{cat.name}</option>) : <><option>Apparel</option><option>Accessories</option><option>Merchandise</option><option>Home Decor</option><option>Lighting</option><option>Furnishings</option></>}</select></label></div>
          ...
          <section className="admin-form-card">
            ...
            <div className="admin-form-grid"><label><span>Price (IDR)</span><div className="admin-input-prefix"><b>Rp</b><input type="number" min="0" value={price} onChange={(event) => setPrice(Number(event.target.value))} /></div></label><label><span>Discount Price</span><div className="admin-input-prefix"><b>Rp</b><input type="number" min="0" value={discountPrice || ""} placeholder="Optional" onChange={(event) => setDiscountPrice(Number(event.target.value))} /></div></label><label><span>Stock Level</span><input type="number" min="0" value={stock} onChange={(event) => setStock(Number(event.target.value))} /></label><label><span>Low Stock Threshold</span><input type="number" min="0" value={lowStockThreshold} onChange={(event) => setLowStockThreshold(Number(event.target.value))} /></label></div>
          </section>
        ...
        <aside className="admin-product-form__aside">
          <section className="admin-form-card">
            ...
            <button className="admin-feature-gold-button admin-publish-button" type="submit">{isEditing ? "Update Produk" : "Publish Now"}</button>
          </section>

          <section className="admin-form-card">
            <h2><Tags size={18} /> Atribut Produk</h2>
            <span className="admin-form-label">Tags</span><div className="admin-tag-list">{(editingProduct?.tags ?? ["Gold", "Handcrafted", "Premium"]).map((tag) => <span key={tag}>{tag}<X size={10} /></span>)}</div>
