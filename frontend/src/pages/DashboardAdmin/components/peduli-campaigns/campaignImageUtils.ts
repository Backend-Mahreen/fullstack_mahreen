...
export const optimizeCampaignImage = async (
  file: File,
  ...
  try {
    const maxWidth = variant === "thumbnail" ? 1200 : 960;
    const maxHeight = variant === "thumbnail" ? 800 : 960;
    ...
    return canvas.toDataURL("image/webp", variant === "thumbnail" ? 0.82 : 0.76);
  } finally {
    bitmap.close();
  }
};
