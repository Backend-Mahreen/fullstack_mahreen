...
const peduliItems: readonly NavigationItem[] = [
  { label: "Beranda", href: "/peduli-mahreen" },
  { label: "Program", href: "/peduli-mahreen#peduli-context" },
  { label: "Dampak", href: "/peduli-mahreen#peduli-story-title" },
  { label: "Kontribusi", href: "/peduli-mahreen#peduli-contribution-title" },
];
...
const isActiveNavigationItem = (currentRoute: string, href: string) => {
  const [hrefPath, hrefHash] = href.split("#");
  const currentHash = currentRoute.includes("#")
    ? currentRoute.slice(currentRoute.indexOf("#") + 1)
    : "";
  const samePath = normalizePath(currentRoute) === normalizePath(hrefPath);

  if (hrefHash) return samePath && currentHash === hrefHash;
  if (normalizePath(href) === "/peduli-mahreen") return samePath && !currentHash;
  return isActiveRoute(currentRoute, href);
};
...
const navbarStyles = `
  ...
  .navbar__ecosystem-button.is-active svg {
    ...
    text-shadow: 0 0 14px rgba(215, 185, 130, 0.38) !important;
  }
...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener("mahreen:navigation", updateCurrentPath);

    return () => {
      window.removeEventListener("popstate", updateCurrentPath);
      window.removeEventListener("hashchange", updateCurrentPath);
      window.removeEventListener("mahreen:navigation", updateCurrentPath);
    };
  }, []);
      ...
      <header className={`site-header${isScrolled ? " is-scrolled" : ""}`}>
        <nav className="navbar" aria-label="Navigasi utama Peduli Mahreen">
          ...
          <div className="navbar__center">
            {peduliItems.slice(0, 3).map((item) => {
              const isActive = isActiveNavigationItem(currentPath, item.href);
              return (
                <a
